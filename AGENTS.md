...
- `page` (`"games" | "music"`): Trạng thái trang hiện tại. Khởi tạo qua hàm `getInitialPage()`:
  ...
  - Ưu tiên 2: Nếu đường dẫn là root `/` hoặc `/index.html` (do CDN / máy chủ tĩnh phục vụ khi F5), đọc từ `sessionStorage.getItem("medonthan_active_page")` (fallback `localStorage`) để giữ nguyên trang người dùng đang xem trong tab hiện tại mà không làm ảnh hưởng các tab khác.
