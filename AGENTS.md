...
- `PAGE_STORAGE_KEY = "medonthan_active_page"`: Key lưu trữ trạng thái trang hiện tại trong `sessionStorage` (cô lập độc lập giữa các tab trình duyệt, tránh việc mở tab mới hoặc F5 bị ảnh hưởng lẫn nhau).
- `page` (`"games" | "music"`): Trạng thái trang hiện tại. Khởi tạo qua hàm `getInitialPage()`:
  ...
  - Ưu tiên 2: Nếu đường dẫn là root `/` hoặc `/index.html` (do CDN / máy chủ tĩnh phục vụ khi F5), đọc từ `sessionStorage.getItem("medonthan_active_page")` để giữ nguyên trang người dùng đang xem trong tab hiện tại mà không làm ảnh hưởng các tab khác; đồng thời tự động xóa sạch key cũ `localStorage.removeItem("medonthan_active_page")` từ các phiên bản trước.
- Tự động chuẩn hóa URL (Clean URL): `useEffect` trong `src/App.tsx` phân tách rõ ràng luồng định tuyến: nếu URL chứa `/music`, lập tức khóa cứng trạng thái `page = "music"`, dọn sạch thanh địa chỉ về `/music` và ngắt hàm (early return) để không bao giờ bị rơi vào fallback `/games`. Các deep link `/app/...` và `/games/...` được bảo toàn nguyên vẹn. Nút điều hướng "♫ MUSIC" và "◀ GAMES" là thẻ ngữ nghĩa `<a href="...">` cho phép người dùng click chuột giữa hoặc chuột phải mở tab mới một cách tự nhiên.
- Script `scripts/postbuild.js`: Không tạo thư mục vật lý `dist/music/` hay `dist/games/` nhằm triệt tiêu hoàn toàn hiện tượng máy chủ web tĩnh (Render / CDN) tự ý phát sinh chuyển hướng `301 Moved Permanently` thêm dấu gạch chéo cuối (`/music/`). Tạo các fallback `dist/music.html`, `dist/games.html`, `dist/404.html`, `dist/200.html` và bàn giao 100% việc điều phối route cho luật Rewrite `/* -> /index.html` trên Render Static Site.
